...
const MenuScreen = () => {
  ...
  const fetchMenuItems = async () => {
    try {
      const response = await fetch('http://localhost:3031/api/menu');
      const data = await response.json();

      // Flatten the menu items data into an array
      const flattenedMenuItems = [];
      for (const category in data) {
        flattenedMenuItems.push(...data[category]);
      }
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.menuItem}>
            <Image style={styles.image} source={{ uri: `data:image/jpeg;base64,${item.pic}` }} />
